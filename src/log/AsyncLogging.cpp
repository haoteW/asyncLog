...
*******************************************************************/
    ...
    {
        //async::LogFile output(basename_,rollSize_,false); //定义日志文件对象 
        std::vector<std::string> buffersToWrite;//  线程函数的局部队列
        latch_.countDown();
        cout << "-------------workthreadfunc------------" <<endl;
        while (running_)
        {
...
async::AsyncLogging *asynclog = nullptr;
...
int main()
{
    asynclog = new async::AsyncLogging("yhping",1024*10);
    async::Logger::setOutput(asyncWriteFile);
    async::Logger::setFlush(asyncFlushFile);
    asynclog->start();
